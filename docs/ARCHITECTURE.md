...
5. `tools/validate_usecases.lua` fuehrt beide Validatoren zuerst aus und deckt danach 1841 Szenarien ueber die aktuell registrierten Module (siehe `tools/usecase_scenarios.lua`) ab; die Regelvalidatoren indizieren die entsprechenden deterministischen Tests.
...
```text
| isiLive                                                 v0.9.273 Open/Close CTRL-F9 [H][V][M][M+][L][X]            |
|------------------------------------------------------------------------------------------------------------------|
...
Zusaetzlich zum Main-Roster-Frame kann `isiLive_ui.lua` optionale Tooling-, Travel-, Mounts- und Addons-Panels links an `GameMenuFrame` anhaengen, und `isiLive_settings.lua` registriert den Blizzard-Settings-Canvas fuer lokalisierte Config- und State-Mirror.
