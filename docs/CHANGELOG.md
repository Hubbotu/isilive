...
- **Styled target-dungeon chat announcement: blue `isiLive` brand prefix, yellow dungeon + key-level ([factory/isiLive_factory_frame_bridge.lua](../factory/isiLive_factory_frame_bridge.lua), [ui/isiLive_status.lua](../ui/isiLive_status.lua)):**
  - User feedback comparing our chat announcement `isiLive: Ziel-Dungeon: Nexuspunkt Xenas +10` against Keystone Polaris's highlighted multi-line output: we should colour the brand prefix and the dungeon/key token so it stands out in the combat log.
